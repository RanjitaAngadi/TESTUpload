...
import {
    ...
    UPDATE_PART_TYPE_COMPONENT_TORQUE_RANGE,
    ConstVariable,
} from "../../common/const";
...
const updateTorqueRangeAjax$ = (URL, { errorHandler }) =>
    new Subject().pipe(
        mergeMap((params) =>
            ampJsonAjax.put(
                URL,
...
export const TorqueRangesConfiguration = () => {
    ...
    return (
        <div className="mb-5">
            ...
            <FetchTorqueRangesConfiguration
...
export const FetchTorqueRangesConfiguration = ({
    partTypeResult,
}) => {
    ...
    useEffect(() => {
        if (partTypeId) {
            setLoader(true)
            ajaxTorqueRangeComponentObsv$.next(partTypeId)
        }
    }, [partTypeId])
            ...
            <Row>
                <Col>
                    <form onSubmit={handleSubmit(onSearch)}>
                        <AMPFieldSet title="Search Torque Components">
...
export const TorqueRangesConfigurationForm = ({
    id,
    componentId,
    partNumber,
    description,
    ...
    const ajaxUpdateTorqueRange$ = useMemo(() => {
        return updateTorqueRangeAjax$(
            DEFAULT_BASE_URL +
            VERSION +
            UPDATE_PART_TYPE_COMPONENT_TORQUE_RANGE, {
            errorHandler: (error) => {
                setLoader(false)
                toast.error(AMPToastConsts.TORQUE_RANGE_CONFIGURATION_SAVE_ERROR, {
                    position: toast.POSITION.TOP_CENTER,
                });
            },
        });
    ...
    useObservableCallback(ajaxUpdateTorqueRange$, (response) => {
        if (response?.status) {
            reset(response?.formData)
            setLoader(false)
            toast.success(AMPToastConsts.TORQUE_RANGE_CONFIGURATION_SAVE_SUCCESS, {
                position: toast.POSITION.TOP_CENTER,
            });
        } else {
            setLoader(false)
    ...
    const onSubmit = (formData) => {
        const request = {
            Id: id,
            TorqueRangeBearingQuantity: parseInt(formData?.quantity),
            RequestedById: parseInt(context?.userId),
            TorqueMinimumValue: formData?.min,
            TorqueMaximumValue: formData?.max

        }
        setLoader(true)
        ajaxUpdateTorqueRange$.next({
            request,
            formData
        })
    }
                ...
                <Col xs={6} sm={3} md={3} lg={2}>
                    <AMPFormLayout>
                        <AMPFieldWrapper
                            ...
                            controlId="quantity"
                            name="quantity"
                            fieldValidationCustom={errors?.quantity?.message ? errors?.quantity?.message : false}
                        >
                            <AMPNumberTextBox
                                ref={register({
                                    required: 'required',
                        ...
                        >
                            <AMPNumberTextBox
                                ref={register({
                                    ...
                                    validate: (min) => parseFloat(getValues('max')) >= parseFloat(min)
                                        || 'should be less than or equal to max value',
                                    required: "required"
                                })}
                                step=".001"
                            />
                        ...
                        >
                            <AMPNumberTextBox
                                ref={register({
                                    ...
                                    validate: (max) => parseFloat(getValues('min')) <= parseFloat(max)
                                        || 'should be more than or equal to min value',
                                    required: "required"
                                })}
                                step=".001"
                            />
