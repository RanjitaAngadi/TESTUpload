...
import {
  ...
  SAVE_INSPECTION_SUBCOMPONENT_POST_CLERANCE_RANGE,
  ConstVariable,
} from "../common/const";
...
const AssemblyClearanceOrTorqueRange = (props) => {
  const {
    title,
    partTypeId,
    customerId,
  ...
  return (
    <>
      {partClearanceTypeComponent.length > 0 ? (
        partClearanceTypeComponent?.map((item, index) => {
          return (
            <AMPAccordion
...
export default AssemblyClearanceOrTorqueRange;


export const AssemblyClearanceForm = ({
  bearing,
  minimumValue,
  maximumValue,
  inspectionDetailId,
  ...
  useObservableCallback(ajaxSaveInspectionClearanceRangeObsv$,
    (response) => {
      if (response?.status) {
        setLoader(false)
